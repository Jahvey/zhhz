...
class UprecordController extends CommonController {
	...
	public function index() {
		//显示收款记录
		$this->assign('data', $this->selectData( $_SESSION['ausernumber'] ));
	...
	Public function updateData() {
		...
		//判断是否已经点击过，防止多次点击
		if ( $_SESSION['abc'] == $unumber ) { $this->error('已经点击过了！', U(C('Mudules') . '/Uprecord/index')); }
		$_SESSION['abc'] = $unumber;
		...
		if ( $res ) {
			...
			$result = M()->table('a_uprecord')->where(array('usernumber'=>$unumber))->data($data)->save();
	...
	Public function selectData( $usernumber ) {
		...
		return M()->table('a_uprecord')->where($whe)->order('applytime DESC')->select();
	}
}